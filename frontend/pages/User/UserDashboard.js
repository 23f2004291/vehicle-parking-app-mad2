...
import ReserveParking from "./ReserveParking.js";
import ActiveReservation from "./ActiveReservation.js";
import ParkingHistory from "./ParkingHistory.js";
...
.content {
  flex: 1;
  padding: 30px;
  background: url('/static/assets/dash-bg.png') no-repeat center center;
...
userStyleSheet.innerText = UserDashboardStyles;
...
export default {
    ...
    <div class="dashboard-container">
      <div class="sidebar">
        ...
        <button class="nav-link" :class="{ active: section === 'lots' }" @click="section = 'lots'">Reserve Parking</button>
        <button class="nav-link" :class="{ active: section === 'active' }" @click="section = 'active'">My Active Reservation</button>
        <button class="nav-link" :class="{ active: section === 'history' }" @click="section = 'history'">Parking History</button>
        <button class="nav-link" :class="{ active: section === 'vehicles' }" @click="section = 'vehicles'">My Vehicles</button>
        <button class="nav-link" :class="{ active: section === 'summary' }" @click="section = 'summary'">Parking Summary</button>
      </div>

      <div class="content">
        <h1>User Dashboard</h1>
        
        <ReserveParking v-if="section === 'lots'" />
        
        <ActiveReservation v-if="section === 'active'" />

        <ParkingHistory v-if="section === 'history'" />

        <MyVehicles v-if="section === 'vehicles'" />

        <MySummary v-if="section === 'summary'" />
